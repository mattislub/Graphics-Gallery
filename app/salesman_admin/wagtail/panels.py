...
class OrderPaymentsPanel(ReadOnlyPanel):
    def classes(self) -> list[str]:
        return ["salesman-order-payments"]
    ...
    def render(self) -> str:
        head = f"""<tr>
            ...
            <td>{_('Date created')}</td>
            </tr>"""

        body = ""
        for payment in self.instance.payments.all():
            body += f"""<tr>
            <td class="title"><h2>{payment.amount}</h2></td>
