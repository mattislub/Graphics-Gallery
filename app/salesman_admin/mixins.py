...
from .utils import format_price, format_json
...
class OrderAdminMixin(BaseAdminMixin):
    ...
    def customer_display(self, obj: BaseOrder) -> str:
        ...
        return mark_safe(f'<a href="{url}">{obj.user.first_name} {obj.user.last_name}</a>')
    ...
    def get_urls(self) -> list[URLPattern | URLResolver]:
        ...
        urls = [
            path(
                ...
                self.admin_site.admin_view(self.my_refund_view),  # type: ignore
                name="shop_order_refund",
            ),
        ] + urls
        return urls

    def my_refund_view(self, request: HttpRequest, object_id) -> HttpResponse:
